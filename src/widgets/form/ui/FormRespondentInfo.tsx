import { FORM_CONSTANT, useFormResultStore } from '@/entities/form';

interface FormRespondentInfoProps {
  responseUserInfos: Form.ReplyResponseDto[];
}

export const FormRespondentInfo = ({
  responseUserInfos,
}: FormRespondentInfoProps) => {
          ...
          {detailUserInfoRightKey.map((key, idx) => {
            if (key === 'graduationYear') {
              ...
              return (
                <li key={`right-${key}${idx}`}>
                  {detailUserInfoRightKeyValue[idx]}:{' '}
                  {responseUserInfos[currentPage - 1].replyUserResponseDto
                    .createdAt
