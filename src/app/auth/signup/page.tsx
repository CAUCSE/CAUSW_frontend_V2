...
const SignUpPage = () => {
  ...
  const onSubmit = async (data: User.SignUpForm) => {
    try {
      const { email, name, password, studentId, admissionYearString, nickname, major, phoneNumber } = data;
    ...
    <div className="bg-white-100 flex min-h-screen w-full items-center justify-center">
      ...
      >
        <div className="sticky left-4 top-0 z-10 mb-4 mt-8 flex w-full items-center bg-white py-2 text-lg">
          <button onClick={handleBack} className="text-black-500 items-left flex hover:text-gray-500">
            <span className="icon-[weui--back-filled] mr-3 mt-[2px] text-2xl font-bold"></span>
            <span>이전</span>
          </button>
        </div>
        <h1 className="mb-6 mt-3 text-center text-2xl font-bold sm:text-3xl">회원가입</h1>

        <div className="flex w-full flex-col sm:w-[600px] sm:flex-row">
          <div className="sm:w-1/2">
            <div className="mb-6 ml-4 mr-4">
              <label className="mb-2 block text-lg font-bold text-gray-700 sm:text-xl">아이디</label>
              <input
            ...
            <div className="mb-6 ml-4 mr-4">
              <label className="mb-2 block text-lg font-bold text-gray-700 sm:text-xl">비밀번호</label>
            ...
            <div className="mb-6 ml-4 mr-4">
              <label className="mb-2 block text-lg font-bold text-gray-700 sm:text-xl">비밀번호 확인</label>
              <div className="flex w-full">
                <input
                  className="mr-4 w-full max-w-md rounded-lg border-2 border-gray-300 p-2"
                  ...
                  onKeyDown={handleKeyDown}
                />
                <button type="button" onClick={togglePasswordConfirmVisibility} className="">
                  {' '}
                  {showPasswordConfirm ? <FaEye></FaEye> : <FaEyeSlash></FaEyeSlash>}
                </button>
              </div>
            ...
            <div className="mb-6 ml-4 mr-4">
              <label className="mb-2 block text-lg font-bold text-gray-700 sm:text-xl">이름</label>
              <input
                className="w-full max-w-md rounded-lg border-2 border-gray-300 p-2"
                type="text"
          ...
          <div className="sm:w-1/2">
            ...
            <div className="mb-6 ml-4 mr-4">
              <label className="mb-2 block text-lg font-bold text-gray-700 sm:text-xl">입학년도</label>
              <select
                className="w-full max-w-md rounded-lg border-2 border-gray-300 p-2"
              ...
              <p className="text-error">{errors?.admissionYearString?.message}</p>
            </div>

            <div className="mb-6 ml-4 mr-4">
              <label className="mb-2 block text-lg font-bold text-gray-700 sm:text-xl">학번</label>
              <input
                className="w-full max-w-md rounded-lg border-2 border-gray-300 p-2"
                type="number"
            ...
            <div className="mb-6 ml-4 mr-4">
              <label className="mb-2 block text-lg font-bold text-gray-700 sm:text-xl">학부/학과</label>
              <input
                className="w-full max-w-md rounded-lg border-2 border-gray-300 p-2"
                type="text"
        ...
        <div className="mb-8 flex w-[calc(100%-30px)] flex-col sm:w-[565px]">
          <div className="-ml-2 flex items-center pl-2">
            ...
            <label htmlFor="terms">
              <label className="cursor-pointer text-lg text-gray-700 underline" onClick={() => setIsModalOpen(true)}>
                약관 읽고 동의하기
              </label>
            </label>
          </div>
