...
const FindEmailPage = () => {
  ...
  const { email, setName, setStudentId, resetFindAccountStore } = useFindAccountStore(
    useShallow(state => ({
      email: state.email,
      ...
      {email !== '' ? (
        <div className="w-full max-w-md rounded-lg bg-white p-8 text-center shadow-md">
          ...
          <div className="mt-4 flex w-full justify-between px-4">
            <button className="mt-5 h-10 w-32 rounded-xl bg-blue-500 text-white" onClick={handleRouteLogin}>
              로그인하기
            </button>
            <button className="mt-5 h-10 w-32 rounded-xl bg-blue-500 text-white" onClick={handleRouteFindPassword}>
              비밀번호 찾기
            </button>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-md rounded-lg bg-white p-8 shadow-md">
          <h2 className="mb-4 text-base font-semibold md:text-xl">아이디(이메일)를 찾고 싶으신가요?</h2>

          <h2 className="mb-4 mt-4 text-xl font-semibold">학번을 입력해주세요.</h2>
          <FormInput
            name="studentId"
            type="text"
          ...
          <h2 className="mb-4 mt-4 text-xl font-semibold">이름을 입력해주세요.</h2>
          <FormInput
            name="name"
            type="text"
