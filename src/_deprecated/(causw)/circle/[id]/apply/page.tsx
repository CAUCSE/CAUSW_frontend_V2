...
import { CircleService, Modal, PreviousButton } from '@/shared';
...
const CircleApplyPage = ({ params: { id } }: { params: { id: string } }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [data1, data2] = await Promise.all([getApplication(id), checkApplication(id)]);
        ...
        <>
          ...
          >
            {form?.questionResponseDtoList
              .sort((questionA, questionB) => questionA.questionNumber - questionB.questionNumber)
                ...
                return (
                  <div key={question.questionId} className="w-3/4">
                    ...
                    >
                      ...
                      {question.isMultiple && <p className="text-[10px] text-[#909090] sm:text-sm">복수 선택 가능</p>}
                    </div>
                              ...
                              <div className="flex gap-2" key={option.optionId}>
                                {question.isMultiple ? (
                                  <input
                                    ...
                                    {...register(`questionReplyRequestDtoList.${questionIdx}.selectedOptionList`)}
                                  />
                                ) : (
                                  <input
                                    type="radio"
            ...
            <button className="min-h-[40px] w-[240px] rounded-md bg-[#6BBEEC] text-white">제출하기</button>
          </form>
          {modalOpen && (
