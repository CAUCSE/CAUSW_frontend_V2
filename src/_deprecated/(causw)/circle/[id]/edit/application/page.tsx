...
const CircleApplicationEdit = ({ params: { id } }: { params: { id: string } }) => {
  ...
  const onSubmit = async (data) => {
    ...
    circleApplicationDto.questionCreateRequestDtoList.forEach((question: Post.QuestionCreateRequestDto) => {
  ...
  const enrolledRegisteredSemesterList = watch('enrolledRegisteredSemesterList');
  const allowAllEnrolledRegisteredSemester = watch('allowAllEnrolledRegisteredSemester');
  const leaveOfAbsenceRegisteredSemesterList = watch('leaveOfAbsenceRegisteredSemesterList');
  const isAllowedLeaveOfAbsence = watch('isAllowedLeaveOfAbsence');
  const allowAllLeaveOfAbsenceRegisteredSemester = watch('allowAllLeaveOfAbsenceRegisteredSemester');
              ...
              <div className="flex h-full w-full flex-col items-center gap-5 lg:items-start">
                <div className="flex w-3/4 min-w-[260px] items-center justify-around rounded-2xl bg-[#FFF5C5] py-10 lg:min-w-[490px]">
                  <div className="grid grid-cols-2 gap-1 lg:grid-cols-5 lg:gap-2">
                    <CustomCheckBox colSize={1} name="재학생" register={register('isAllowedEnrolled')} />
                    <CustomCheckBox
                ...
                {errors.isAllowedEnrolled && <p className="text-red-500">{errors.isAllowedEnrolled.message}</p>}
                ...
                <div className="flex w-3/4 min-w-[260px] items-center justify-around rounded-2xl bg-[#FDE4DE] py-10 lg:min-w-[490px]">
                  <div className="grid grid-cols-2 gap-x-5 gap-y-1 lg:grid-cols-5 lg:gap-2">
                    <CustomCheckBox colSize={1} name="휴학생" register={register('isAllowedLeaveOfAbsence')} />
                    <CustomCheckBox
                      colSize={isViewPointLg ? 4 : 1}
                      name="졸업생"
                    ...
                    <CustomCheckBox
                      colSize={1}
                      name="상관없음"
                      register={register('allowAllLeaveOfAbsenceRegisteredSemester')}
                    />
                    {SemesterOptions.map((grade, idx) => (
                      <CustomCheckBox
                        colSize={grade.colSize as 1 | 2 | 3 | 4 | 5}
                        ...
                        name={grade.name}
                        key={idx}
                        register={register('leaveOfAbsenceRegisteredSemesterList')}
                      />
                    ))}
                  </div>
                </div>
                {errors.isAllowedLeaveOfAbsence && (
                  <p className="text-red-500">{errors.isAllowedLeaveOfAbsence.message}</p>
                )}
                <hr className="w-3/4 min-w-[260px] border-dashed border-black lg:min-w-[490px]" />
                <div className="flex w-3/4 min-w-[260px] flex-col bg-white lg:min-w-[500px]"></div>
              </div>
              {fields.map((field, idx) => {
                return <CircleApplyQuestion key={field.id} index={idx} removeQuestion={() => remove(idx)} />;
              })}
              <button
                type="button"
