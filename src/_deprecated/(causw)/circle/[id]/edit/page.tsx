...
import { Header, LoadingComponent, ProfileImage, SubHeader } from '@/entities';
...
const CircleDetailEdit = ({ params: { id } }: { params: { id: string } }) => {
  ...
  const handleChange = debounce(
    <K extends keyof Circle.CircleRequestDto>(
      ...
      newCircle[key] =
        key === 'recruitEndDate'
          ? ((event.target.value + 'T23:59:59.999999') as Circle.CircleRequestDto[K])
  ...
  const submitHandler = () => {
    ...
    if (mainImg) formData.append('mainImage', new Blob([mainImg], { type: mainImg.type }), mainImg.name);
  ...
  return (
    <>
      <div className="mt-8 ml-[3%] grid h-[800px] w-[90%] grid-cols-[1fr_1fr_1fr] grid-rows-[1fr_1fr_1fr_1fr_1fr_4fr_6fr_1fr_1fr_1fr_1fr_1fr] gap-4 md:mt-[6%] lg:h-5/6">
        ...
        <label className="row-span-4 flex min-h-36 min-w-36 items-center overflow-hidden" htmlFor="upload">
          <img
            src={mainImg ? URL.createObjectURL(mainImg) : (circle.mainImage ?? '/images/signin-logo.png')}
            alt={'Circle Image'}
