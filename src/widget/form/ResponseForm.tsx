    ...
    >
      {form?.questionResponseDtoList
        ...
        .map((question: Post.QuestionResponseDto, questionIdx: number) => {
          return <ResponseQuestion key={question.questionId} question={question} questionIdx={questionIdx} />;
        })}
      <button type="submit" className="min-h-[40px] w-[240px] rounded-md bg-[#6BBEEC] text-white">
        제출하기
      </button>
    </form>
  );
