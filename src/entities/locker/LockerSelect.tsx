...
export const LockerSelect = ({ locker }: LockerSelectProps) => {
  ...
  return (
    <button
      className={`h-16 w-16 ${clickedLockerId === locker.id ? "border-4 border-[#76C6D1] scale-110 shadow-md" : ""} ${locker.isActive ? "bg-white" : locker.isMine ? "bg-[#76C6D1]" : "bg-[#D9D9D9]"} ${locker.isActive && "border border-[#BABABA]"} text-[#686868]`}
