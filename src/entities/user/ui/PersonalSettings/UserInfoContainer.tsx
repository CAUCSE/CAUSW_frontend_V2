    ...
    <div className="grid grid-cols-2 gap-4 lg:flex lg:gap-2">
      <div className="lg:mr-32">
        ...
        {userData.academicStatus !== 'GRADUATED' && (
          <>
            <UserInfo
              title="등록 완료 학기"
              content={userData.currentCompletedSemester}
            />
            <UserInfo
              title="본 학기 학생회비 적용 여부"
              content={feeInfo.studentCouncilFeeStatus}
            />
          </>
        )}
      </div>
      <div>
        ...
        {userData.academicStatus !== 'GRADUATED' && (
          <>
            <UserInfo
              title="납부한 학생회비 학기 차수"
              content={feeInfo.paidFeeSemesters}
            />
            <UserInfo
              title="남은 학생회비 차수"
              content={feeInfo.remainingFeeSemesters}
            />
          </>
        )}
      </div>
...
const UserInfo: React.FC<UserInfoProps> = ({ title, content }) => {
  return (
    <div className="mb-4 max-w-40">
      ...
      <p className="overflow-hidden break-words text-gray-700">
        {content || <>&nbsp;</>}
      </p>
    </div>
  );
