...
export const MessageItem = ({ msg, showAvatar }: Chat.MessageItemProps) => (
  ...
  >
    ...
    <div
      className={`flex w-full flex-col gap-2 ${msg.isMine ? 'justify-end' : 'justify-start'}`}
    >
      {showAvatar && !msg.isMine && (
        <div className="text-[15px] font-medium text-black">김재학</div>
      )}

      <div
        className={`flex w-full flex-row gap-1 ${msg.isMine ? 'justify-end' : 'justify-start'}`}
      >
        {msg.isMine ? (
          <>
            <div className="mt-auto text-xs font-normal text-gray-400">
              {msg.timestamp}
            </div>
