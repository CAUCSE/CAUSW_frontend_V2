...
export function useUserReportedList(
  type: ReportType,
  userId: string,
  pageNum = 0,
) {
  return useQuery<ReportedItem[]>({
    queryKey:
      type === 'post'
        ? reportQueryKey.userPosts(userId, pageNum)
        : reportQueryKey.userComments(userId, pageNum),
    queryFn: () =>
      type === 'post'
        ? getUserReportedPosts(userId, pageNum)
        : getUserReportedComments(userId, pageNum),
    enabled: !!userId,
    staleTime: 30_000,
  });
}
