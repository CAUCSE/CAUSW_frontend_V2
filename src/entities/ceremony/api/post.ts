...
export const cancelCeremonyRegist = async ({
  ceremonyId,
}: {
  ceremonyId: string;
}) => {
...
export const createCeremonyNotificationSetting = async (
  payload: Ceremony.NotificationSettingPayload,
) => {
  try {
    ...
    if (axios.isAxiosError(error)) {
      const errorMessage =
        error.response?.data?.message || '알림 설정 생성에 실패했습니다.';
