...
export const createPost = async ({
  ...
  const { data }: { data: CreatePostResponse } = await FORMAPI.post(
    '/api/v1/posts',
    formData,
  );
  return data.id;
...
export const createPostWithForm = async ({
  postData,
  attachImageList,
}: CreatePostParams) => {
  const formData = new FormData();
...
export const subscribePost = async ({
  postId,
}: {
  postId: Post.PostDto['id'];
}) => {
  const { data }: { data: Post.PostSubscribeResponseDto } = await API.post(
    `/api/v1/posts/subscribe/${postId}`,
  );
  return data;
};
