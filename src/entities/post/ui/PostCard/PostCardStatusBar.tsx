    ...
    <div className="flex items-center gap-2">
      <div className="flex gap-2 lg:gap-4">
        <div className="flex items-center gap-1 text-red-500 sm:gap-2">
          <ThumbsUp size={16} className="md:size-6" />
          <p className="text-base md:text-xl">{totalCount(post.numLike)}</p>
        </div>
        <div className="hidden items-center gap-1 text-yellow-500 sm:flex">
          <Star size={16} className="md:size-6" />
          <p className="text-base md:text-xl">{totalCount(post.numFavorite)}</p>
        </div>
        <div className="flex items-center gap-1 text-blue-300 sm:gap-2">
          <MessageCircle size={16} className="md:size-6" />
          <p className="text-base md:text-xl">{totalCount(post.numComment)}</p>
        </div>
      </div>
      <Divider vertical />
      <div className="flex items-center gap-2">
        <VoteIcon className={clsx(post.isPostVote ? 'text-[#FFC71B]' : 'text-[#B4B1B1]', 'h-5 w-5 md:h-6 md:w-6')} />
        <FormIcon className={clsx(post.isPostForm ? 'text-[#FFC71B]' : 'text-[#B4B1B1]', 'h-5 w-5 md:h-6 md:w-6')} />
      </div>
      <Divider vertical />
