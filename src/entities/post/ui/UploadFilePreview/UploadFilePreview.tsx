...
export const UploadFilePreview = () => {
  ...
  return (
    <>
      <p className="border-t-comment-bw border-file-preview-border pt-2 text-sm text-gray-400">
        {MESSAGES.FILE_TYPE_INFO}
      </p>
      <div className="flex h-fit w-full shrink-0 grow gap-4 overflow-x-scroll p-4">
        {[...selectedFileList].reverse().map((file, index) => (
          <div
            key={index}
            className="relative h-24 w-24 shrink-0 border border-black bg-cover bg-center lg:h-32 lg:w-32"
          >
